...
int main()
{
	...
	__asm
	{
		...
		and edx, edx		// Проверка на равенство остатка 0
		jnz skip1			
		push 999			// Добавление в стек значения, соответствующего нулевому
		jmp skip11
		skip1:
		push edx			// Очередная тетрада -> стек
		skip11:
		inc ecx				// Увеличение счетчика итераций
		and eax, eax;		// Проверка на равенство 0
		jnz cyc1 
		cmp ecx, 2			// Проверка на прохождение двух итераций
		jz skip2			// Пропускаем, если прошло две итерации
		push 999			// Если две итерации не прошло - добавляем 999 в стек			

		skip2:
		xor ebx,ebx			// Обнуляем множитель
		xor ecx, ecx		// Обнуляем счетчик

		cyc2:
		pop esi		 //Достаем тетраду из стека
		
		cmp esi, 999	 //Сравнение тетрады с 999
